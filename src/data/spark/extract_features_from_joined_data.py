...
# $ spark-submit --master yarn --deploy-mode cluster src/data/spark/extract_features_from_joined_data.py
...
def extract_features(input_tup):
    k, v = input_tup
    alexa_rank, records = v
    dash_idx = k.rfind('-')
...
joined_rdd = joined_rdd.map(extract_features)
